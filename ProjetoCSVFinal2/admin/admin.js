...
function initDetailPanel() {
  if (document.getElementById("detailPanel")) return;
  ...
  panel.setAttribute("aria-label", "Detalhes da resposta");
  panel.innerHTML = `
    <div class="detail-panel__header">
      <div class="detail-panel__title-group">
        <div class="detail-panel__title" id="detailTitle">—</div>
        <div class="detail-panel__sub" id="detailSub">—</div>
      </div>
      <button class="detail-panel__close" id="detailClose" aria-label="Fechar painel">
  ...
  document.addEventListener("keydown", (e) => {
    if (e.key === "Escape" && detailPanelEl && detailPanelEl.classList.contains("is-open")) {
      closeDetailPanel();
    }
  });
}
...
function openDetailPanel(row) {
  ...
  document.getElementById("detailTitle").textContent = row.officeName || "—";
  document.getElementById("detailSub").textContent = dateLabel;

  const sys = Number(row.ratingSystem);
  const sup = Number(row.ratingSupport);
  const usedTags = (row.modulesUsed || []).map((m) => `<span class="tag">${escapeHtml(m)}</span>`).join("") || "<span class='detail-empty'>—</span>";
  const interestTags = (row.modulesInterest || []).map((m) => `<span class="tag">${escapeHtml(m)}</span>`).join("") || "<span class='detail-empty'>—</span>";

  const supportSection = row.needSupport === "sim" ? `
    <div class="detail-section detail-section--alert">
      <div class="detail-section__title">
        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="12" cy="12" r="10"/><path d="M12 8v4M12 16h.01"/></svg>
        Pedido de apoio imediato
      </div>
      <div class="detail-field">
        <span class="detail-field__label">Contato informado</span>
        <span class="detail-field__value">${escapeHtml(row.contactInfo || "—")}</span>
      </div>
      ${row.supportDescription ? `
      <div class="detail-field">
        <span class="detail-field__label">Descrição / Dúvida do cliente</span>
        <p class="detail-field__text">${escapeHtml(row.supportDescription)}</p>
      </div>` : ""}
    </div>
  ` : "";
  ...
  document.getElementById("detailBody").innerHTML = `
    <div class="detail-section">
      ...
      <div class="detail-field">
        <span class="detail-field__label">Escritório</span>
        <span class="detail-field__value">${escapeHtml(row.officeName || "—")}</span>
      </div>
      <div class="detail-field">
        <span class="detail-field__label">CPF / CNPJ</span>
        <span class="detail-field__value">${escapeHtml(row.cnpj || "—")}</span>
      </div>
      <div class="detail-field">
        <span class="detail-field__label">Responsável</span>
        <span class="detail-field__value">${escapeHtml(row.userName || "—")}</span>
      </div>
    </div>

    <div class="detail-section">
      <div class="detail-section__title">Módulos</div>
      <div class="detail-field">
        <span class="detail-field__label">Em uso</span>
        <div class="detail-field__tags">${usedTags}</div>
      </div>
      <div class="detail-field">
        <span class="detail-field__label">Interesse em novos</span>
        <div class="detail-field__tags">${interestTags}</div>
      </div>
    </div>

    <div class="detail-section">
      <div class="detail-section__title">Avaliações</div>
      <div class="detail-field">
        <span class="detail-field__label">Nota do sistema</span>
        <div class="detail-field__score-row">
          <span class="score-pill ${scoreClass(sys)}">${isNaN(sys) ? "—" : sys + " / 10"}</span>
          ${row.justifySystem ? `<p class="detail-field__text">${escapeHtml(row.justifySystem)}</p>` : ""}
        </div>
      </div>
      <div class="detail-field">
        <span class="detail-field__label">Nota do suporte</span>
        <div class="detail-field__score-row">
          <span class="score-pill ${scoreClass(sup)}">${isNaN(sup) ? "—" : sup + " / 10"}</span>
          ${row.justifySupport ? `<p class="detail-field__text">${escapeHtml(row.justifySupport)}</p>` : ""}
        </div>
      </div>
    </div>
  ...
  detailOverlayEl.classList.add("is-open");
...
function renderTable(rows) {
  ...
  if (!rows.length) {
    tbody.innerHTML = `<tr class="empty-row"><td colspan="11">Nenhuma resposta encontrada para esse filtro.</td></tr>`;
    return;
  }
  ...
  tbody.innerHTML = sorted.map((r, i) => {
    ...
    return `
      <tr>
        ...
        <td>
          <button class="btn-ver" data-idx="${i}" type="button">
...
// Delegação de eventos para os botões "Ver" na tabela
mainEl.addEventListener("click", (e) => {
  const btn = e.target.closest(".btn-ver");
  if (btn) {
    const idx = Number(btn.dataset.idx);
    if (!isNaN(idx) && displayedRows[idx]) {
      openDetailPanel(displayedRows[idx]);
    }
  }
});
